...
Modified sections:
  - III. Code Quality: Added "하드코딩 금지" rule
  - VI. TDD Cycle: Clarified feature → dev PR workflow
  - Version Control & Recovery: Strengthened rollback requirements
Added sections:
  - VIII. No Hardcoding (새 원칙)
Removed sections: None
Templates requiring updates:
  - .specify/templates/plan-template.md ✅ (no changes needed - Constitution Check covers)
...
- **하드코딩 금지**: 설정값, URL, 상수는 반드시 config 또는 환경변수로 관리
...
- **feature 브랜치에서 TDD 완료 후에만 dev로 PR 생성**
...
- **PR 생성**: feature 브랜치에서 TDD 완료 후 dev로 PR
...
### VIII. No Hardcoding (하드코딩 금지)

모든 설정값은 외부에서 주입 가능해야 한다.

- **URL, 엔드포인트**: 환경변수 또는 config 파일로 관리
- **API 키, 비밀번호**: 절대 코드에 포함 금지, .env 필수
- **매직 넘버**: 상수로 정의하고 이름으로 의미 전달
- **타임아웃, 재시도 횟수**: config로 분리
- **수수료율, 임계값**: 설정 파일로 관리 (런타임 변경 가능)
...
# GOOD - 설정으로 분리
fee_rate = config.FEE_RATE  # from .env or config.py
url = settings.UPBIT_API_URL
```

**Rationale**: 하드코딩은 환경 변경 시 코드 수정이 필요하고, 실수로 민감 정보가 커밋될 위험이 있다. 설정 분리는 배포 유연성과 보안을 동시에 확보한다.
...
main ────────────────────► 프로덕션 (완벽하게 검증된 코드만)
  ...
  └─► dev ──────────────► 개발 통합 (자유롭게 테스트)
        ...
        │                   - CI 통과 필수
        │
        └─► feature/* ──► 기능 개발 (TDD 사이클 필수)
            L-work          - TDD: RED → GREEN → REFACTOR
            H-work          - 모든 테스트 통과 후에만 PR
            P-work          - PR → dev 머지 → 검증 → main
```

### Feature → Dev PR 워크플로우 (필수)
...
Push: 커밋 3-5개마다 또는 기능 완료 시
  ↓
PR: TDD 완료 + 모든 테스트 통과 후 dev로 생성
```

**Claude 작업 시 필수**:
1. 작업 완료 후 `git add` + `git commit`
2. 즉시 `git push`
3. TDD 완료 + 테스트 통과 후 PR 생성 (`gh pr create --base dev`)
...
## Version Control & Recovery (버전 관리 철저)

**핵심 원칙**: 언제든 이전 상태로 돌아갈 수 있어야 한다.
...
5. **배포 전 태그 필수**: 프로덕션 배포 전 반드시 버전 태그 생성

### Rollback 보장 규칙

- **모든 변경은 되돌릴 수 있어야 한다**
- DB 마이그레이션: 반드시 down migration 포함
- API 변경: 하위 호환성 유지 또는 버전 분리
- 설정 변경: 이전 설정 백업 필수
- 배포: 이전 버전으로 즉시 롤백 가능한 구조
...
| 배포 롤백 | `git checkout <prev-tag>` → 재배포 |
...
**Version**: 1.3.0 | **Ratified**: 2025-12-13 | **Last Amended**: 2025-12-17
